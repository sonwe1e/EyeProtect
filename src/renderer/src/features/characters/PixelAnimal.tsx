...
export type PixelAnimalAction = 'idle' | 'react' | 'eye' | 'walk' | 'combined' | 'celebrate';
...
export const PixelAnimal = memo(function PixelAnimal({
  animal,
  action,
  label
}: {
  ...
  const [showEarsB, setShowEarsB] = useState(false);
  const [showTongue, setShowTongue] = useState(false);
  const [wrapperFx, setWrapperFx] = useState<'is-react' | 'is-celebrate' | null>(null);
  ...
  useEffect(() => {
    ...
    let running: PixelAnimalAction = 'idle';
    ...
    const playBlink = (times: number): void => {
      for (let i = 0; i < times; i += 1) {
        later(() => setBlink(true), 40 + i * 420);
        later(() => setBlink(false), 170 + i * 420);
      }
    };

    const playEye = (duration = 2_600): void => {
      clear();
      later(() => setPose('pose-gaze'), 30);
      if (!reduced.matches) {
        later(() => playBlink(2), 260);
        later(() => playBlink(1), 1_400);
      }
      later(stopAction, duration);
    };

    const playWalk = (duration = 3_200): void => {
      clear();
      setShowTail(animal === 'cat');
      setShowEarsB(animal === 'rabbit');
      if (reduced.matches) {
        later(stopAction, 500);
        return;
      }
    ...
    const playReact = (): void => {
      clear();
      setShowTongue(true);
      setWrapperFx('is-react');
      later(stopAction, 720);
    };

    const playCelebrate = (): void => {
      clear();
      setShowTongue(true);
      setWrapperFx('is-celebrate');
      later(stopAction, 640);
    };

    const run = (next: string): void => {
      const kind = (['idle', 'react', 'eye', 'walk', 'combined', 'celebrate'].includes(next)
        ? next
        : 'idle') as PixelAnimalAction;
      if (kind === running && kind !== 'react' && kind !== 'celebrate') return;
      running = kind;
  ...
  const cream = '#fff0d4';
  const pink = '#eaa4a9';
  const svgClass = ['pa-art', pose ?? '', action === 'idle' && !pose ? 'pa-idle' : ''].filter(Boolean).join(' ');

  return (
    <div
      ref={hostRef}
      className={`pixel-animal ${wrapperFx ?? ''}`.trim()}
      role="img"
      aria-label={label}
      data-animal={animal}
      data-action={action}
    >
      <svg className={svgClass} viewBox="0 0 64 64" shapeRendering="crispEdges" aria-hidden="true">
        <path fill={ink} opacity=".15" d="M18 59H46V61H18Z" />
        {cat ? (
          <>
            <g style={{ display: showTail ? 'none' : undefined }}>
              <path fill={ink} d="M43 49H49V45H53V51H51V55H43Z" />
              <path fill={mid} d="M45 51H49V49H51V51H49V53H45Z" />
            </g>
            <g style={{ display: showTail ? undefined : 'none' }}>
              <path fill={ink} d="M44 47H50V41H54V49H52V53H44Z" />
              <path fill={mid} d="M46 49H50V45H52V49H50V51H46Z" />
            </g>
          </>
        ) : null}
        <g className="pa-bob">
        ...
        <g className="pa-head-bob">
          {rabbit ? (
            <>
              <path fill={ink} style={{ display: showEarsB ? 'none' : undefined }} d="M18 8H25V10H27V28H18ZM37 10H39V8H46V28H37Z" />
              <path fill={ink} style={{ display: showEarsB ? undefined : 'none' }} d="M19 8H25V10H27V28H18V12H19ZM37 10H40V8H46V12H47V28H37Z" />
          ...
          <g transform={rabbit ? 'translate(0 4)' : undefined}>
            ...
            {blink ? (
              <path fill={ink} d="M22 29H27V31H22ZM37 29H42V31H37Z" />
            ) : (
              <>
                <path fill={ink} d="M23 26H27V32H23ZM37 26H41V32H37Z" />
                <path fill="#fff9f0" d="M23 26H25V28H23ZM37 26H39V28H37Z" />
              </>
            )}
            ...
            {showTongue ? <path fill={pink} d="M31 37H33V39H31Z" /> : null}
          </g>
        </g>
